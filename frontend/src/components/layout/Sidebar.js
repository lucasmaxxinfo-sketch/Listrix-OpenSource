...
export const Sidebar = () => (
  <aside data-testid="app-sidebar" className="hidden md:flex md:flex-col md:w-[260px] shrink-0 border-r border-border bg-card/60 lx-noise">
    ...
    <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto lx-scroll">
      {NAV.map(({ to, label, icon: Icon, testid }) => (
        <NavLink key={to} to={to} data-testid={testid}
          className={({ isActive }) => `group relative flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm transition-colors ${isActive ? "bg-muted/60 text-foreground shadow-[0_0_0_1px_rgba(255,122,26,0.18)]" : "text-muted-foreground hover:bg-muted/40 hover:text-foreground"}`}>
    ...
    <div className="px-5 py-4 border-t border-border"><div className="flex items-center gap-2 text-xs text-muted-foreground"><span className="h-2 w-2 rounded-full bg-[hsl(var(--lx-green))] shadow-[0_0_8px_hsl(var(--lx-green))]" /><span className="font-mono">AI engine · online</span></div></div>
  </aside>
);
